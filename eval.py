...
def test(args, model, test_loader, model_path):
    checkpoint = torch.load(
        model_path, map_location="cuda:{}".format(torch.cuda.current_device())
    )
    ...
    with torch.no_grad():
        ...
        for inputs, labels in tqdm(
            test_loader, total=len(test_loader), ncols=70, leave=False, unit="step"
        ):
            inputs = inputs.cuda()
    ...
    video_results = cal_metrics(
        video_labels, video_predictions, threshold=0.5
    )  # 'best' 'auto' or float
    print(
        "Test result: V_Acc: {:.2%}, V_Auc: {:.4} V_EER:{:.2%} F_Acc: {:.2%}, F_Auc: {:.4} F_EER:{:.2%}".format(
            video_results.ACC,
            video_results.AUC,
            video_results.EER,
            frame_results.ACC,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device", "-dv", type=int, default=0, help="specify which GPU to use"
    )
    parser.add_argument(
        "--model_path", "-md", type=str, default="models/train/models_params_0.tar"
    )
    parser.add_argument(
        "--resume", "-rs", type=int, default=-1, help="which epoch continue to train"
    )
