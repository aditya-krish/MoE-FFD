...
def train(args, model, optimizer, train_loader, valid_loader, scheduler, save_dir):
    ...
    for epoch in range(args.resume + 1, args.epochs):
        ...
        with torch.enable_grad():
            ...
            for i, (inputs, labels) in enumerate(train_loader):
                ...
                if global_step % args.record_step == 0:
                    ...
                    log.info(
                        "Training state: Epoch [{:0>3}/{:0>3}], Iteration [{:0>3}/{:0>3}], Loss: {:.4f} Acc:{:.2%} time:{}m {}s".format(
                            epoch + 1,
                            args.epochs,
                            i + 1,
                            len(train_loader),
        ...
        log.info(
            "valid result: Epoch [{:0>3}/{:0>3}], V_Acc: {:.2%}, V_Auc: {:.4} V_EER:{:.2%} F_Acc: {:.2%}, F_Auc: {:.4} F_EER:{:.2%}".format(
                epoch + 1,
                args.epochs,
                video_results.ACC,
                video_results.AUC,
