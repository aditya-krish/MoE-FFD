...
def createConvFunc(op_type):
    assert op_type in ["cv", "cd", "ad", "rd", "scd"], "unknown op type: %s" % str(
        op_type
    )
    if op_type == "cv":
        return F.conv2d
...
class Conv2d_Adapter(nn.Module):
    def __init__(
        ...
        self.adapter_conv.weight.data[:, :, 1, 1] += torch.eye(
            adapter_dim, dtype=torch.float
        )
...
class Adapter_MoElayer(nn.Module):
    ...
    def __init__(
        ...
        self.w_gate = nn.Parameter(
            torch.zeros(dim, self.num_experts), requires_grad=True
        )
...
class LoRA_MoElayer(nn.Module):
    ...
    def __init__(
        self, dim, lora_dim=[8, 16, 32, 48, 64, 96, 128], noisy_gating=True, k=1
    ):  #
        ...
        self.w_gate = nn.Parameter(
            torch.zeros(dim, len(Lora_a_experts)), requires_grad=True
        )  # here we need avg token dim
        self.w_noise = nn.Parameter(
            torch.zeros(dim, len(Lora_a_experts)), requires_grad=True
        )
        self.register_buffer("mean", torch.tensor([0.0]))
